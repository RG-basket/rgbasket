...
const NewArrivals = () => {
  ...
  const { products: contextProducts, lastUpdated } = useappcontext();
  ...
  // Real-time updates when new products arrive
  useEffect(() => {
    fetchNewArrivals();
  }, [lastUpdated]); // Re-run when products are updated in context

  // Auto-refresh every 2 minutes to check for new products
  useEffect(() => {
