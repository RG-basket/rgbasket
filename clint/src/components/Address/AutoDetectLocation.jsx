import React, { useState, useEffect } from "react";
import { useGeolocated } from "react-geolocated";
import { useAppContext } from "../../context/AppContext";
import { Geolocation } from '@capacitor/geolocation';
import { Capacitor } from '@capacitor/core';

const AutoDetectLocation = () => {
  const { serviceAreas } = useAppContext();
  const [location, setLocation] = useState(null);
  const [isServiceable, setIsServiceable] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  const [isManualTriggered, setIsManualTriggered] = useState(false);

  // Web Geolocation Hook
  const {
    coords,
    positionError,
    getPosition, 
  } = useGeolocated({
    positionOptions: {
      enableHighAccuracy: true,
      timeout: 15000,
      maximumAge: 0,
    },
    userDecisionTimeout: 10000,
    suppressLocationOnMount: true, 
  });

  const [showRationale, setShowRationale] = useState(false);

  // Function to handle the manual click
  const handleDetectClick = () => {
    // Show rationale BEFORE requesting location permission (Google Play requirement)
    setShowRationale(true);
  };

  const handleRationaleConfirm = async () => {
    setShowRationale(false);
    setIsManualTriggered(true);
    setIsLoading(true);

    if (Capacitor.isNativePlatform()) {
      try {
        const permissions = await Geolocation.checkPermissions();
        if (permissions.location === 'denied' || permissions.location === 'prompt') {
          await Geolocation.requestPermissions();
        }

        const coordinates = await Geolocation.getCurrentPosition({
          enableHighAccuracy: true,
          timeout: 10000
        });

        if (coordinates.coords) {
          await reverseGeocode(coordinates.coords);
        }
      } catch (error) {
        console.error("Native geolocation failed:", error);
        setIsLoading(false);
      }
    } else {
      getPosition();
    }
  };

  useEffect(() => {
    if (coords && isManualTriggered && !Capacitor.isNativePlatform()) {
      reverseGeocode(coords);
    }
  }, [coords, isManualTriggered]);

  const reverseGeocode = async ({ latitude, longitude }) => {
    setIsLoading(true);
    try {
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/api/geocode/reverse?lat=${latitude}&lon=${longitude}`
      );

      if (res.ok) {
        const data = await res.json();

        if (data.success && data.location) {
          const detectedLocation = {
            area: data.location.area,
            district: data.location.district,
            state: data.location.state,
            pincode: data.location.pincode,
          };

          setLocation(detectedLocation);

          const match = serviceAreas.find(
            (entry) => entry.pincode === detectedLocation.pincode
          );

          setIsServiceable(!!match);
        }
      }
    } catch (error) {
      console.error("Geocoding failed:", error);
    } finally {
      setIsLoading(false);
      setIsManualTriggered(false);
    }
  };

  return (
    <div className="text-xs text-gray-700 leading-tight max-w-full">

      {/* Location Permission Rationale Dialog — Google Play requirement */}
      {showRationale && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50 px-4">
          <div className="bg-white rounded-2xl shadow-2xl p-5 max-w-xs w-full">
            <div className="flex items-center gap-2 mb-3">
              <span className="text-2xl">📍</span>
              <h3 className="text-sm font-black text-gray-900">Location Access</h3>
            </div>
            <p className="text-xs text-gray-600 leading-relaxed mb-4">
              RG Basket needs your location to check if we deliver to your area. Your location is only used to verify serviceability and is never shared with third parties.
            </p>
            <div className="flex gap-2">
              <button
                onClick={() => setShowRationale(false)}
                className="flex-1 py-2 rounded-xl border border-gray-200 text-xs font-bold text-gray-500"
              >
                Cancel
              </button>
              <button
                onClick={handleRationaleConfirm}
                className="flex-1 py-2 rounded-xl bg-emerald-600 text-white text-xs font-bold"
              >
                Allow
              </button>
            </div>
          </div>
        </div>
      )}

      {isLoading ? (
        <div className="flex items-center gap-1.5 text-blue-600">
          <div className="w-3 h-3 border-2 border-blue-600 border-t-transparent rounded-full animate-spin" />
          <span>Detecting...</span>
        </div>
      ) : location ? (
        <div
          onClick={handleDetectClick}
          className="cursor-pointer hover:bg-gray-50 p-1 rounded transition-colors"
        >
          <div className="whitespace-nowrap overflow-hidden text-ellipsis font-medium">
            {location.area}, {location.pincode}
          </div>
          <div className={`font-semibold ${isServiceable ? "text-green-600" : "text-amber-600"}`}>
            {isServiceable ? (
              <span className="flex items-center gap-1">
                <span className="w-1.5 h-1.5 bg-green-600 rounded-full" />
                Serviceable
              </span>
            ) : (
              <span className="flex items-center gap-1">
                <span className="w-1.5 h-1.5 bg-amber-600 rounded-full" />
                Not Serviceable
              </span>
            )}
          </div>
        </div>
      ) : (
        <button
          onClick={handleDetectClick}
          className="flex items-center gap-1.5 text-blue-600 hover:text-blue-700 font-medium transition-colors"
          title="Click to detect location"
        >
          <span>📍</span>
          <span>Check Serviceability</span>
        </button>
      )}

      {positionError && isManualTriggered && (
        <div className="text-[10px] text-red-500 mt-1">
          Permission denied or error.
        </div>
      )}
    </div>
  );
};

export default AutoDetectLocation;